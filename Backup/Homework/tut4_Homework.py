...
class Soldier(pygame.sprite.Sprite):
    def __init__(self, char_type, x, y, scale, speed, ammo, flip):
        ...
        # Use to set the time gap from one Bullet to another.
        ...
        # Use to set the Sprite facing side.abs
        self.flip = flip
...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, x, y, direction):
        ...
        # Flag to check if the bullet has bounced back.
        self.bounced = False
    ...
    def update(self):
        ...
        if self.rect.right < 0 or self.rect.left > SCREEN_WIDTH:
            # If the Bullet is off-screen, make it bounce back.
            self.direction += -1
            self.bounced = True

        # Apply gravity to the vertical position if it hasn't bounced back yet.
        if not self.bounced:
            # self.rect.y += GRAVITY
            self.rect.x += (self.direction * self.speed)
        if self.bounced:
            # If the Bullet is bounced. Reduce the Y value with gravity.
            # Add a multiplier to set the falling distance(Higher is better).
            self.rect.y += GRAVITY * 3
        # Check for collision with the floor.
        if self.rect.bottom > 300:
            # Adjust the vertical position to prevent the bullet from going below the floor.
            self.rect.bottom = 300
            # Stop the bullet from moving vertically.
            self.speed = 0
...
player = Soldier('player', 200, 200, 3, 5, 20, False)
enemy = Soldier('enemy', 500, 245, 3, 5, 20, True)

# Setting the Game Loop variable to true.
run = True
...
while run:
    ...
    # Update Enemy actions if the Player is alive.
    if enemy.alive:
        # Update the Enemy action to 'idle' (action code 0).
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_w and player.alive:
                player.jump = True
            # Trigger a jump if the 'b' key is pressed and the enemy is alive.
            if event.key == pygame.K_l and enemy.alive and not enemy.in_air:
                enemy.jump = True
