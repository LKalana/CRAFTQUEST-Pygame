...
TILE_SIZE = 40  # Define the size of each tile in pixels
...
BG = (0, 0, 0)  # Define the background color
...
class Grenade(pygame.sprite.Sprite):  # Define a Grenade class
    def __init__(self, x, y, direction):  # Constructor method
        ...
        self.vel_y = -11  # Initialize the vertical velocity
        ...
        self.bounce_count = 0  # Initialize the bounce count

    def update(self):  # Define a method to update the grenade
        self.vel_y += GRAVITY  # Add gravity to the vertical velocity
        dx = self.direction * self.speed  # Calculate the change in x-coordinate
        dy = self.vel_y  # Calculate the change in y-coordinate

        if self.rect.bottom + dy > 300:  # If the bottom of the rectangle plus dy is greater than 300
            dy = 300 - self.rect.bottom  # Set dy to 300 minus the bottom of the rectangle
            self.vel_y = -self.vel_y * 0.5  # Reverse velocity with some loss due to bounce
            self.bounce_count += 1  # Increment the bounce count

        if self.rect.left + dx < 0 or self.rect.right + dx > SCREEN_WIDTH:  # If the grenade hits the left or right boundary
            self.direction *= -1  # Reverse the direction
            dx = self.direction * self.speed  # Recalculate dx
            self.bounce_count += 1  # Increment the bounce count

        self.rect.x += dx  # Update the x-coordinate
        self.rect.y += dy  # Update the y-coordinate

        if self.bounce_count >= 2:  # If the grenade has bounced twice
            self.speed = 0  # Stop horizontal movement
            self.vel_y = 0  # Stop vertical movement
            # Countdown timer for explosion
        self.timer -= 1  # Decrease the timer
        if self.timer <= 0:  # If the timer reaches zero
            ...
            # Inflict damage to nearby entities
            if abs(self.rect.centerx - player.rect.centerx) < TILE_SIZE * 2 and \
                    abs(self.rect.centery - player.rect.centery) < TILE_SIZE * 2:
                player.health -= 50  # Decrease the player's health
            if abs(self.rect.centerx - enemy.rect.centerx) < TILE_SIZE * 2 and \
                    abs(self.rect.centery - enemy.rect.centery) < TILE_SIZE * 2:
                enemy.health -= 100  # Decrease the enemy's health

class Explosion(pygame.sprite.Sprite):  # Define an Explosion class
    def __init__(self, x, y, scale):  # Constructor method
        pygame.sprite.Sprite.__init__(self)  # Call the superclass constructor
        self.images = []  # Initialize a list to store explosion images
        for num in range(1, 6):  # Iterate over image numbers
            img = pygame.image.load(f'img/explosion/exp{num}.png').convert_alpha()  # Load the image
            img = pygame.transform.scale(img, (int(img.get_width() * scale), int(img.get_height() * scale)))  # Scale the image
            self.images.append(img)  # Add the image to the list
        self.frame_index = 0  # Initialize the frame index
        self.image = self.images[self.frame_index]  # Set the current image
        self.rect = self.image.get_rect()  # Get the rectangle of the image
        self.rect.center = (x, y)  # Set the center of the rectangle
        self.counter = 0  # Initialize a counter for animation

    def update(self):  # Define a method to update the explosion
        EXPLOSION_SPEED = 4  # Define the speed of the explosion animation
        self.counter += 1  # Increment the counter
        if self.counter >= EXPLOSION_SPEED:  # If the counter reaches the explosion speed
            self.counter = 0  # Reset the counter
            self.frame_index += 1  # Move to the next frame
            if self.frame_index >= len(self.images):  # If all frames have been displayed
                self.kill()  # Remove the explosion
            else:  # Otherwise
                self.image = self.images[self.frame_index]  # Update the current image
...
while run:  # Main game loop
    ...
    explosion_group.update() # Update the explosions
    bullet_group.draw(screen)  # Draw the bullets
    grenade_group.draw(screen)  # Draw the grenades
    explosion_group.draw(screen) # Draw the explosions
